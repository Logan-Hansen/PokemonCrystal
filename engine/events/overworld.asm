...
; graphics bug if don't have badge and TM and examine whirlpool
TryWhirlpoolOW::
; Step 1
	ld de, ENGINE_GLACIERBADGE
	ld b, CHECK_FLAG
...
TryCutOW::
; Step 1 - check for badge
	ld de, ENGINE_HIVEBADGE
	call CheckEngineFlag
	jr c, .cant_cut
; Step 2 - check for hm in bag
	ld a, HM_CUT
	ld [wCurItem], a
	ld hl, wNumItems
	call CheckItem
...
; Step 3 - check if pokemon can learn cut
	ld d, CUT
	call CheckPartyCanLearnMove
       and a
	jr z, .yes

; Step 4 - check if a pokemon knows cut
	ld d, CUT
	call CheckPartyMove
	jr c, .cant_cut
.yes
